...
export default function VoteDetailPage() {
  ...
  // Додајемо стање за гласање по подтачкама
  const [subItemVotes, setSubItemVotes] = useState<Record<string, VoteOption>>({});
  const [subItemVoteConfirmed, setSubItemVoteConfirmed] = useState<Record<string, boolean>>({});
  const [subItemVoteRegistered, setSubItemVoteRegistered] = useState<Record<string, boolean>>({});
    ...
    if (
      ...
      const voteOnProposal = async () => {
        try {
          ...
          console.log("Governor адреса:", await governor.getAddress());
          console.log("Проверавам стање предлога ID:", selectedProposal.id.toString());
          
          const proposalSnapshot = await governor.proposalSnapshot(selectedProposal.id);
          const proposalDeadline = await governor.proposalDeadline(selectedProposal.id);
          // const currentBlock = await signer.provider.getBlockNumber();
          
          console.log("Snapshot блок:", proposalSnapshot.toString());
          console.log("Deadline блок:", proposalDeadline.toString());
          ...
          // За вишеслојне предлоге када гласамо у начелу, означавамо да је главни глас дат
          if (selectedProposal.isMultilayered && selectedVote === "for") {
            // Ажурирамо main vote result како бисмо омогућили гласање за подтачке
            if (selectedProposal) {
              selectedProposal.mainVoteResult = "passed";
            }
          }
        } catch (error) {
  ...
  // Додајемо ефекат за гласање по подтачкама
  useEffect(() => {
    if (!activeSubItemId || !selectedProposal?.isMultilayered || !signer) return;
    
    const selectedSubItemVote = subItemVotes[activeSubItemId];
    const isConfirmed = subItemVoteConfirmed[activeSubItemId];
    
    if (selectedSubItemVote && selectedSubItemVote !== "didntVote" && isConfirmed) {
      const voteOnSubItem = async () => {
        try {
          console.log("Гласање за подтачку ID:", activeSubItemId);
          
          // Овде бисмо имали имплементацију за гласање на подтачкама
          // У стварном случају, ово би био засебан позив на блокчејн
          // За сада симулирамо да је успешно
          await new Promise(resolve => setTimeout(resolve, 1000));
          
          // Ажурирамо локално стање
          setSubItemVoteRegistered(prev => ({
            ...prev,
            [activeSubItemId]: true
          }));
          
          // Ажурирамо предлог да приказује глас корисника
          if (selectedProposal && selectedProposal.subItems) {
            const subItem = selectedProposal.subItems.find(item => item.id === activeSubItemId);
            if (subItem) {
              subItem.yourVote = selectedSubItemVote;
            }
          }
          
          toast({
            title: "Успешно сте гласали за подтачку",
            description: `Успешно сте гласали: ${convertVoteOptionToString(selectedSubItemVote)}`,
          });
        } catch (error) {
          console.error("Грешка при гласању за подтачку:", error);
          toast({
            title: "Грешка при гласању за подтачку",
            description: 
              error instanceof Error
                ? error.message
                : "Дошло је до непознате грешке при гласању за подтачку.",
            variant: "destructive",
          });
        }
      };
  ...
  }, [signer, activeSubItemId, subItemVotes, subItemVoteConfirmed, selectedProposal, toast]);
  ...
  const handleSubItemVote = (subItemId: string, vote: VoteOption) => {
    setActiveSubItemId(subItemId);
    setSubItemVotes(prev => ({
      ...prev,
      [subItemId]: vote
    }));
  };

  const confirmSubItemVote = (subItemId: string) => {
    setSubItemVoteConfirmed(prev => ({
      ...prev,
      [subItemId]: true
    }));
  };

  // Проверавамо да ли је предлог усвојен у начелу - потребно за вишеслојне предлоге
  const isMainVotePassed = () => {
    if (!selectedProposal?.isMultilayered) return false;
    return selectedProposal.mainVoteResult === "passed" || selectedProposal.yourVote === "for";
  };
            ...
            <div className="flex flex-col gap-4">
              <div className="flex items-start justify-between gap-2 flex-wrap">
                ...
                <div className="flex gap-2 items-center">
                  ...
                  {selectedProposal.isMultilayered && (
              ...
              <div className="space-y-6">
                ...
                <Card>
                  <CardHeader>
                    <CardTitle>
                      {selectedProposal.isMultilayered ? "Гласање у начелу" : "Детаљи предлога"}
                    </CardTitle>
                    <CardDescription>
                      {selectedProposal.isMultilayered 
                        ? "Прво је потребно гласати о предлогу у начелу" 
                        : "Информације о предлогу и гласању"}
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4">
                      <div>
                        <h3 className="text-sm font-medium mb-2">Опис предлога</h3>
                        <p className="text-sm">{selectedProposal.description}</p>
                      </div>

                      {/* Додатне информације о предлогу */}
                        ...
                        <div className="bg-muted rounded-md p-3">
                          ...
                          <div className="space-y-1 text-sm">
                            <div className="flex justify-between py-1 border-b">
                              <span>Аутор предлога</span>
                              <span className="font-medium">
                                {selectedProposal.author}
                              </span>
                            </div>
                            <div className="flex justify-between py-1 border-b">
                              <span>Тип предлога</span>
                              <span className="font-medium">
                                {selectedProposal.isMultilayered 
                                  ? "Вишеслојни предлог" 
                                  : "Стандардни предлог"}
