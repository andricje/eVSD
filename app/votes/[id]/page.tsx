...
async function endProposalVoting(
  ...
  try {
    ...
    console.log(
      "Simuliramo zatvaranje glasanja za predlog:",
      proposalId.toString()
    );
      ...
      <Card className="w-full max-w-md mx-4">
        ...
        <CardContent>
          <div className="text-center p-4">
            ...
            <p className="text-muted-foreground mt-2">
              Nakon potvrde glasanje će biti zatvoreno i rezultati će biti
              konačni. Ova akcija ne može biti poništena.
            </p>
          </div>
        </CardContent>
        <CardFooter className="flex justify-end gap-2">
...
const StatusBadge = ({
  ...
  if (status === "closed") {
    ...
    expiresAt &&
    hasVotingTimeExpired({ closesAt: expiresAt } as Proposal)
  ) {
    return (
      <Badge
        variant="outline"
...
export default function ProposalDetails() {
        ...
        if (fetchedProposal) {
          ...
          if (signerAddress) {
            const isCurrentUserAuthor =
              fetchedProposal.author.toLowerCase() ===
  ...
  const quorumPercentage = proposal
    ? (Number(totalVotes) / Number(QUORUM)) * 100
    : 0;
  const quorumRemaining = proposal
    ? Math.max(0, Number(QUORUM) - Number(totalVotes))
    : 0;
  const isQuorumReached = proposal
    ? Number(totalVotes) >= Number(QUORUM)
    : false;
  ...
  const isMultilayered =
    proposal?.isMultilayered &&
    proposal.subItems &&
    proposal.subItems.length > 0;
  ...
  const handleEndVotingConfirm = async () => {
    ...
    try {
      ...
      setError(
        "Došlo je do greške prilikom zatvaranja glasanja. Pokušajte ponovo."
      );
    } finally {
          ...
          <Card className="mb-8 border-border/40 shadow-sm">
              ...
              <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-2">
                ...
                {isMultilayered && (
                  <div className="flex items-center gap-1 bg-blue-50 text-blue-700 px-3 py-1.5 rounded-lg text-sm">
                    ...
                    <span>
                      Višeslojni predlog ({proposal.subItems?.length} podtačaka)
                    </span>
                  </div>
                )}
              </div>
            ...
            <CardContent>
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    {quorumRemaining > 0 && (
                      <span className="text-xs text-muted-foreground">
                        (još {quorumRemaining} glasova)
                      </span>
                    )}
                    {isQuorumReached && (
                      <Badge
                        variant="outline"
                        className="bg-green-500/10 text-green-700 ml-2 text-xs"
                      >
              ...
              {proposal.status === "open" && !isQuorumReached && (
                <Alert className="mb-6 bg-blue-50 border-blue-200">
                  ...
                  <AlertDescription>
                    Vreme za glasanje počinje tek kada se dostigne kvorum od{" "}
                    {QUORUM} glasova. Trenutno ima {totalVotes} od potrebnih{" "}
                    {QUORUM} glasova.
                  </AlertDescription>
                </Alert>
              )}
              ...
              {proposal.status === "closed" && (
                ...
                >
                  ...
                  <AlertTitle>
                    Glasanje je završeno - Predlog je{" "}
                    {proposal.votesFor > proposal.votesAgainst
                      ? "prihvaćen"
                      : "odbijen"}
                  </AlertTitle>
            ...
            {isVotingEnabled && (
              <CardFooter className="flex gap-2 pt-2 border-t">
                <h3 className="font-medium text-sm w-full mb-2">
                  Glasanje o predlogu u načelu
                </h3>
              </CardFooter>
            )}
