...
export function VotingHistory({ activity }: { activity: UserActivityEvent[] }) {
  ...
  if (activity.length === 0) {
    return (<><h2 className="text-xl font-semibold">{STRINGS.userActivity.votingHistory.title}</h2>
      <div className="rounded-xl bg-gray-50 p-8 text-center">
        <Check className="mx-auto h-10 w-10 text-gray-400" />
        <h3 className="mt-4 text-lg font-medium">{STRINGS.userActivity.votingHistory.noHistoryTitle}</h3>
  ...
  return (<><h2 className="text-xl font-semibold">{STRINGS.userActivity.votingHistory.title}</h2>
    <div className="space-y-3">{votingActivity.map((evt, index) => (
      <div
        key={index}
