...
function ProposalDescriptionBody({ proposal }: { proposal: Proposal }) {
    switch (proposal.status) {
        case "open":
            return <div className="flex items-center justify-between text-xs text-gray-500 pt-3 border-t">
                <div>{STRINGS.userActivity.userProposals.addedAt}: {formatDate(proposal.dateAdded)}</div>
        ...
        case "closed":
            return <div className="flex items-center justify-between text-xs text-gray-500 pt-3 border-t">
                <div>{STRINGS.userActivity.userProposals.closedAt}: {formatDate(proposal.dateAdded)}</div>
            </div>;
        case "cancelled":
            return <div className="flex items-center justify-between text-xs text-gray-500 pt-3 border-t">
...
export function UserProposals({ proposals, user }: { proposals: Proposal[], user: User | null }) {
    ...
    return (<div className="space-y-6">
        <h2 className="text-xl font-semibold">{STRINGS.userActivity.userProposals.title}</h2>

        {userProposals.length === 0 ? (
            <div className="rounded-xl bg-gray-50 p-8 text-center">
                <Timer className="mx-auto h-10 w-10 text-gray-400" />
                <h3 className="mt-4 text-lg font-medium">{STRINGS.userActivity.userProposals.noProposalsTitle}</h3>
            ...
            <div className="space-y-8">
                ...
                {activeProposals.length > 0 && (
                    <div>
                        <h3 className="text-lg font-medium mb-4">{STRINGS.userActivity.userProposals.activeProposals}</h3>
                        <div className="grid gap-4 sm:grid-cols-2">
                            {activeProposals.map((proposal) => (
                                <ProposalDescription proposal={proposal} key={proposal.id.toString()} />
                            ))}
                        </div>
                    </div>
                )}
