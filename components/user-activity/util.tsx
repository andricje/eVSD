...
export function getTextForActivityEvent(activityEvt: UserActivityEvent) {
  if (IsUserActivityVote(activityEvt)) {
    const voteOptionTranslated = getTranslatedVoteOption(
      activityEvt.voteEvent.vote
    );
    ...
    switch (activityEvt.type) {
      case "Create":
        return `${STRINGS.userActivity.timeLine.created}: ${activityEvt.proposal.title}`;
      case "Delete":
        return `${STRINGS.userActivity.timeLine.deleted}: ${activityEvt.proposal.title}`;
    }
  }
}
