...
async function main() {
  ...
  console.log(
    "Пребацујем токене са другог налога:",
  ...
  // Провера стања
  const initialBalanceSecond = await token.balanceOf(
    await secondAccount.getAddress()
  );
  const initialBalanceFirst = await token.balanceOf(
    await deployer.getAddress()
  );

  console.log(
    `Почетно стање другог налога: ${ethers.formatUnits(initialBalanceSecond, 18)} токена`
  );
  console.log(
    `Почетно стање првог налога: ${ethers.formatUnits(initialBalanceFirst, 18)} токена`
  );
  ...
  // Провера новог стања
  const newBalanceSecond = await token.balanceOf(
    await secondAccount.getAddress()
  );
  const newBalanceFirst = await token.balanceOf(await deployer.getAddress());

  console.log(
    `Ново стање другог налога: ${ethers.formatUnits(newBalanceSecond, 18)} токена`
  );
  console.log(
    `Ново стање првог налога: ${ethers.formatUnits(newBalanceFirst, 18)} токена`
  );

  // Повезујемо се на токен уговор као први налог
  const tokenAsFirst = EvsdToken__factory.connect(
    tokenArtifacts.address,
    deployer
  );
  ...
  console.log(
    `Гласачка моћ првог налога: ${ethers.formatUnits(votingPower, 18)}`
  );
