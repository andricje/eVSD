...
async function main() {
  ...
  console.log(
    `Почетно стање циљане адресе: ${ethers.formatUnits(initialBalance, 18)} токена`
  );

  // Шаљемо токене
  console.log(
    `Шаљем ${ethers.formatUnits(ONE_TOKEN, 18)} токена на адресу ${targetAddress}...`
  );
  ...
  console.log(
    `Ново стање циљане адресе: ${ethers.formatUnits(newBalance, 18)} токена`
  );
