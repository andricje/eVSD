...
async function main() {
  ...
  const proposalId =
    "0x7048f99ecf384ac60cad3cb4650decdf8d468c48ed2ce914217326ca8d4a216d";
  ...
  try {
    ...
    console.log(
      `Стање предлога: ${stateNumber} (${proposalStates[stateNumber] || "Непознато"})`
    );
    ...
    if (stateNumber === 1) {
      ...
      console.log(
        `Рок за гласање: ${deadline} (остало још ${Number(deadline) - currentBlock} блокова)`
      );
      ...
      if (stateNumber === 0) {
        ...
        console.log(`Тренутни блок: ${currentBlock}`);
        console.log(
          `Гласање ће бити активно за ${Number(snapshot) - currentBlock} блокова`
        );
      }
    }
  } catch (error) {
