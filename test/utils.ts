...
export async function delegateVoteToSelf(evsdToken: EvsdToken, voter: ethers.Signer) {
...
export async function castVotes(proposalServices: ProposalService[], voteItem: VotableItem, votes: VoteOption[]) {
    for (let i = 0; i < votes.length; i++) {
        const proposalService = proposalServices[i];
        await proposalService.voteForItem(voteItem, votes[i]);
    }
}
export async function fastForwardTime(days: number, hours: number, minutes: number) {
...
export function assertProposalEqual(uiProposal: UIProposal, proposal: Proposal) {
...
export function assertVoterVoteRecordedCorrectly(voteItem: VotableItem, voter: User, vote?: VoteOption) {
    if (vote) {
        expect(
            voteItem.userVotes.get(voter)
        ).to.be.equal(vote);
    } else {
        expect(
            voteItem.userVotes
        ).to.not.contain(voter);
    }
}